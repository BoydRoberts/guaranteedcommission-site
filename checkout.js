// /checkout.js - build 2025-12-24 (Commission change: add &cc=1 URL hint for signature.html safety)
document.addEventListener("DOMContentLoaded", function() {
  console.log("[checkout.js] build 2025-12-24 - Commission change cc=1 URL hint");
  ...
  function isAgentNovemberPromoActive() {
    try { return data.payer === "agent" && (new Date()).getFullYear() === 2025; }
  ...
  if (payNowBtnEl) {
    payNowBtnEl.addEventListener("click", async function() {
      ...
      try {
        ...
        var successSignature = window.location.origin + "/signature.html" +
          (listingId ? "?id=" + encodeURIComponent(listingId) + "&session_id={CHECKOUT_SESSION_ID}"
                    : "?session_id={CHECKOUT_SESSION_ID}");

        // ✅ Add &cc=1 hint for commission changes
        var successSignatureCC = window.location.origin + "/signature.html" +
          (listingId ? "?id=" + encodeURIComponent(listingId) + "&cc=1&session_id={CHECKOUT_SESSION_ID}"
                    : "?cc=1&session_id={CHECKOUT_SESSION_ID}");

        var successSellerDetail = window.location.origin + "/seller-detail.html" +
        ...
        var successAgent = window.location.origin + "/agent-detail.html" +
          (listingId ? "?id=" + encodeURIComponent(listingId) + "&session_id={CHECKOUT_SESSION_ID}"
                    : "?session_id={CHECKOUT_SESSION_ID}");

        var successUrl;
        if (data.payer === "agent") successUrl = successAgent;
        else if (isCommissionChange) successUrl = successSignatureCC;  // ✅ cc=1
        ...
        if (data.upgrades.banner && !paidUpgrades.banner && !promo) items.push({ price: PRICE_IDS.BANNER, quantity: 1 });
        if (data.upgrades.pin && !paidUpgrades.pin) { if (!promo) items.push({ price: PRICE_IDS.PIN, quantity: 1 }); }
        else if (data.upgrades.premium && !paidUpgrades.premium && !paidUpgrades.pin) { if (!promo) items.push({ price: PRICE_IDS.PREMIUM, quantity: 1 }); }
