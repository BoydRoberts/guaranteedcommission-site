...
export default async function handler(req, res) {
  ...
  try {
    ...
    // FIX #1: Pass through success URL as-is (client already includes {CHECKOUT_SESSION_ID})
    // Guard: Ensure client included the placeholder (prevents silent breakage)
    const hasPlaceholder = String(successUrl).includes("{CHECKOUT_SESSION_ID}");
    if (!hasPlaceholder) {
      return res.status(400).json({ error: "successUrl must include session_id={CHECKOUT_SESSION_ID}" });
    }
